...
int main() {
    ...
    // create a window
    GLFWwindow *window;
    if((window = glfwCreateWindow(width, height, "compute_shader_nbody", 0, 0)) == 0) {
    ...
    ComputeShader accelerateShader("7.4.accelerate.comp");
    ComputeShader integrateShader("7.4.integrate.comp.spv");
    ComputeShader tiledAccelerateShader("7.4.tiled_accelerate.comp");

    Shader ourShader("7.4.nbody.vs", "7.4.nbody.fs", "7.4.nbody.gs");
    // // shader source code
    ...
    // physical parameters
    // ubo needs to be used for spir-v
    // meanwhile, vulkan version GLSL is neccessary
    float dt = 1.0f/60.0f;
    ubo.dt = dt;
    GLuint ubo_ptr;
    glGenBuffers(1, &ubo_ptr);
	glBindBuffer(GL_UNIFORM_BUFFER, ubo_ptr);
	glBufferData(GL_UNIFORM_BUFFER, sizeof(UBO), &ubo, GL_STREAM_DRAW);
    // glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(UBO), &ubo);
	glBindBufferBase(GL_UNIFORM_BUFFER, 2, ubo_ptr);
    CHECK_ERROR();

    // // setup uniforms
    // tiledAccelerateShader.use();
    // tiledAccelerateShader.setFloat("dt", dt);

    // accelerateShader.use();
    // accelerateShader.setFloat("dt", dt);
