...
int main()
{
    ...
    // ------------------------------------
    Shader screenShader("../../src/7.in_practice/6.displace_mapping/7.6.screen.vs", "../../src/7.in_practice/6.displace_mapping/7.6.screen.fs"); 
    Shader diplacementMappingShader("../../src/7.in_practice/6.displace_mapping/7.6.displacement_mapping.vs", "../../src/7.in_practice/6.displace_mapping/7.6.displacement_mapping.fs");
    ...
    glBindTexture(GL_TEXTURE_2D, noiseTexture); // all upcoming GL_TEXTURE_2D operations now have effect on this texture object
    ...
    unsigned char *data = stbi_load(FileSystem::getPath("src/7.in_practice/6.displace_mapping/noise.png").c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    // int width, height, nrChannels;
    // The FileSystem::getPath(...) is part of the GitHub repository so we can find files on any IDE/platform; replace it with your own image path.
    data = stbi_load(FileSystem::getPath("src/7.in_practice/6.displace_mapping/color.png").c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
